...
import { getSchedule, getSimpleTeamInfo } from "@/lib/espn";
...
export const metadata: Metadata = {
  title: `Women's Basketball Schedule 2025-26 | ${SITE_NAME}`,
  description: "Clemson Tigers Women's Basketball 2025-26 schedule with game times, opponents, and results.",
  openGraph: {
    title: `Women's Basketball Schedule 2025-26 | ${SITE_NAME}`,
    description: "Clemson Tigers Women's Basketball 2025-26 schedule with game times, opponents, and results.",
    type: "website",
    url: `${SITE_URL}/schedule/womens-basketball`,
  },
};
...
interface GameOverride {
  month: number;
  day: number;
  opponentMatch: string; // Partial match (case-insensitive)
  status: string;
  className?: string;
}

const GAME_STATUS_OVERRIDES: GameOverride[] = [
  // UL Monroe - November 25, 2025 - Postponed
  { month: 11, day: 25, opponentMatch: "monroe", status: "POSTPONED", className: "text-amber-600" },
];

// Check if a game has a status override
function getGameOverride(date: string, opponentName: string, opponentAbbr: string) {
  const gameDate = new Date(date);
  const month = gameDate.getMonth() + 1; // getMonth() is 0-indexed
  const day = gameDate.getDate();
  const opponentLower = (opponentName || "").toLowerCase();
  const abbrLower = (opponentAbbr || "").toLowerCase();

  for (const override of GAME_STATUS_OVERRIDES) {
    if (
      override.month === month &&
      override.day === day &&
      (opponentLower.includes(override.opponentMatch.toLowerCase()) ||
       abbrLower.includes(override.opponentMatch.toLowerCase()))
    ) {
      return { status: override.status, className: override.className };
    }
  }
  return null;
}
...
export default async function WomensBasketballSchedulePage() {
  let games: SimpleScheduleGame[] = [];
  let teamRecord: string | undefined;

  try {
    const [schedule, teamInfo] = await Promise.all([
      getSchedule("womensBasketball"),
      getSimpleTeamInfo("womensBasketball"),
    ]);
    if (schedule?.events) {
    ...
    console.error("Failed to fetch women's basketball schedule:", error);
  }
    ...
    <>
      ...
      <BreadcrumbSchema
        items={[
          ...
          { name: "Women's Basketball", url: `${SITE_URL}/schedule/womens-basketball` },
        ]}
      />
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="relative z-10 text-center">
            ...
            <nav className="text-base md:text-lg text-gray-300 uppercase">
              ...
              <span className="mx-2">/</span>
              <span className="text-[var(--clemson-orange)]">Women&apos;s Basketball</span>
            </nav>
          </div>
        </div>
      </section>
...
function ScheduleRow({ game }: { game: SimpleScheduleGame }) {
  ...
  // Check for status override (postponed, canceled, etc.)
  const statusOverride = getGameOverride(
    game.date,
    game.opponent.displayName || game.opponent.name,
      ...
      <div className="flex-1 bg-white flex items-center px-4 md:px-6 gap-3 md:gap-6">
        ...
        <div className="flex flex-col justify-center">
          {isCompleted ? (
            ...
            <span className={cn(
              "text-base md:text-lg font-black uppercase",
              statusOverride.className || "text-amber-600"
            )}>
